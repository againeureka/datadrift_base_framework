...
# --- 3. DDOC NLP Plugin 구현 (ddoc/plugins/ddoc-plugin-nlp/nlp_impl.py) ---
...
class DDOCNlpPlugin:
    ...
    def transform_apply(self, input_path: str, transform: str, args: Dict[str, Any], output_path: str) -> Optional[Dict[str, Any]]:
        ...
        if transform == "text.tokenize":
            ...
            result = {"ok": True, "written": output_path, "transform": transform, "provider": "ddoc_nlp", "summary": f"토큰 {len(tokens)}개 생성"}
        
        elif transform == "text.normalize":
            # 다중 공백 및 양쪽 끝 공백 제거
            norm = " ".join(text.split())
            Path(output_path).write_text(norm, encoding="utf-8")
            result = {"ok": True, "written": output_path, "transform": transform, "provider": "ddoc_nlp", "summary": f"텍스트 정규화 완료"}
    ...
    def ddoc_get_metadata(self) -> Dict[str, Any]:
        """플러그인의 메타데이터를 제공하는 훅."""
        return {"name": "ddoc_nlp", "implemented": ["transform_apply"]}
